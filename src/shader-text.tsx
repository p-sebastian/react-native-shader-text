...
import type {TShaderRunProps, TShaderTextProps} from './shader-text.type'
import {
  DEFAULT_BLUR_OUT_DURATION,
  DEFAULT_DELAY,
  DEFAULT_DURATION,
  DEFAULT_FONT_SIZE,
  DEFAULT_REVEAL_BLUR_RADIUS,
  DEFAULT_REVEAL_DURATION,
  flattenShaderTextChildren,
  resolveTextColor,
  validateShaderTextProps,
} from './shader-text.util'

export const ShaderText = (props: TShaderTextProps): ReactElement => {
  const {
    accessibilityLabel,
    allowFontScaling = true,
    animation,
    blurOutDuration = DEFAULT_BLUR_OUT_DURATION,
    children,
    color,
    delay = DEFAULT_DELAY,
    ...
    onAnimationEnd,
    onAnimationStart,
    revealBlurRadius = DEFAULT_REVEAL_BLUR_RADIUS,
  ...
  if (Platform.OS !== 'ios') throw new Error('ShaderText is iOS-only.')
  ...
  const onContentHeightChange = callback((height: number) => {
  ...
  const onNativeAnimationEnd = callback((result: {finished: boolean}) => onAnimationEnd?.(result))
  ...
  return (
    <NativeShaderText
      ...
      blurOutDuration={blurOutDuration}
      combinedAccessibilityLabel={combinedAccessibilityLabel}
      delay={delay}
      duration={duration}
      ...
      fontWeight={weight}
      italic={italic}
      onAnimationEnd={onNativeAnimationEnd}
      onAnimationStart={onNativeAnimationStart}
      onContentHeightChange={onContentHeightChange}
      pointerEvents="none"
      revealBlurRadius={revealBlurRadius}
      revealDuration={revealDuration}
      segments={segments}
      style={[styles.block, hasExplicitHeight ? undefined : {height: contentHeight}, style]}
      textAlign={textAlign}
      textColor={textColor}
    />
  )
...
export const ShaderRun = (_props: TShaderRunProps): ReactNode => {
